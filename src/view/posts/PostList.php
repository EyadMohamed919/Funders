...
for ($i=0; $i < count($posts); $i++) { 
    ...
    if($posts[$i]->getStatus() == "active")
    {
        $jsonArray = [$posts[$i]->getId(), $posts[$i]->getTitle(), 
        $posts[$i]->getDescription(), $posts[$i]->getCalculateAmount($posts[$i]->getId())
        , $posts[$i]->getTargetAmount(), $posts[$i]->getStatus(), CategoryController::getCategory($posts[$i]->getCategoryId())->getName()];
        ...
        echo "<div onclick='checkDetails(" . $jsonPost . ")' class='post-card'>";
            ...
            echo "<div class='post-bar-container'>";
                ...
                echo "<h4>" . $posts[$i]->getCalculateAmount($posts[$i]->getId()) . "EGP/";
